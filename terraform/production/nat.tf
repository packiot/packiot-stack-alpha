...
# (the dedicated DB EC2 in database.tf). Mirrors staging's nat.tf: ~$3/mo vs
# ~$33/mo for an AWS Managed NAT Gateway. Production upgrade path once HA
# matters: replace aws_instance.nat + the aws_route in vpc.tf with
# aws_nat_gateway + an EIP.
#
...
# AL2023 Graviton (t4g) uses ens5, not eth0, and ships nftables only — install
# iptables-nft before adding masquerade rules.

resource "aws_security_group" "nat" {
  name        = "packiot-production-nat"
  ...
  tags = { Name = "packiot-production-nat" }
}
