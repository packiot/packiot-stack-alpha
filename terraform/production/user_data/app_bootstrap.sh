...
#
# Production variant of staging's user_data/app_bootstrap.sh. The differences
# are minimal — only the S3 key prefix (scripts/production/ vs scripts/) and
# log prefix (production vs staging).
set -euo pipefail
...
log() { echo "[bootstrap-prod $(date -u +%T)] $*" | tee -a "$LOG" >> /dev/console 2>/dev/null || true; }

log "=== Packiot Production app bootstrap starting ==="
...
until aws s3 ls "s3://${state_bucket}/scripts/production/" --region ${aws_region} > /dev/null 2>&1; do
...
aws s3 cp "s3://${state_bucket}/scripts/production/app_init.sh" /opt/packiot-app-init.sh \
    --region ${aws_region}
