...
resource "aws_s3_bucket" "historian" {
  bucket = "packiot-production-historian-${data.aws_caller_identity.current.account_id}"
  tags = {
    Name    = "packiot-production-historian"
...
resource "aws_iam_policy" "historian_writer" {
  name = "packiot-production-historian-writer"
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
      {
...
resource "aws_iam_role_policy_attachment" "app_historian_writer" {
  role       = aws_iam_role.app.name
  policy_arn = aws_iam_policy.historian_writer.arn
}

# ── IAM: Superset/BI read (Athena + Glue catalog + S3 read) ───────────────────
# Grants the Superset box's instance role everything PyAthena needs to run
# queries against the historian workgroup with NO static keys (SQLAlchemy URI in
# configs/superset/databases/historian_athena.yaml uses the instance role). This
# activates when the Superset box is applied (superset.tf); it is codified here so
# the historian is query-ready the moment Superset comes up — same cross-resource
# follow-up posture as superset.tf's other gated steps.
resource "aws_iam_policy" "superset_historian_reader" {
  name = "packiot-production-superset-historian-reader"
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
      {
        ...
        Resource = [
          "arn:aws:athena:${var.aws_region}:${data.aws_caller_identity.current.account_id}:workgroup/${aws_athena_workgroup.historian.name}",
      ...
      },
      {
        ...
        Resource = [
          ...
          "arn:aws:glue:${var.aws_region}:${data.aws_caller_identity.current.account_id}:database/${aws_glue_catalog_database.historian.name}",
          "arn:aws:glue:${var.aws_region}:${data.aws_caller_identity.current.account_id}:table/${aws_glue_catalog_database.historian.name}/*",
        ]
      },
      {
        Sid      = "ReadColdStoreData"
        Effect   = "Allow"
        Action   = ["s3:GetObject", "s3:ListBucket", "s3:GetBucketLocation"]
        Resource = [aws_s3_bucket.historian.arn, "${aws_s3_bucket.historian.arn}/*"]
      },
      {
        Sid      = "AthenaResultsReadWrite"
        Effect   = "Allow"
        Action   = ["s3:PutObject", "s3:GetObject"]
        Resource = "${aws_s3_bucket.historian.arn}/athena-results/*"
      },
    ]
  })
}

resource "aws_iam_role_policy_attachment" "superset_historian_reader" {
  role       = aws_iam_role.superset.name
  policy_arn = aws_iam_policy.superset_historian_reader.arn
}
