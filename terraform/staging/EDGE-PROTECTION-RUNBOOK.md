# Edge Protection Runbook — CloudFront + WAF + ACM in front of staging

**Terraform:** `terraform/staging/edge.tf` (+ minimal edits to `main.tf`,
`dns.tf`, `security_groups.tf`).

This runbook takes staging from "every `<svc>.staging.packiot.app` A-record points
...
This is the staging port of `terraform/production/EDGE-PROTECTION-RUNBOOK.md`. It is
**simpler than production**: there is no `dash.packiot.app` SAN/alias (dash is a
single cross-env page hosted on PROD, not a staging service), so the cert covers only
the wildcard + apex and every validation record lands in the staging **child** zone —
there is no cross-zone parent case.
...
| Resource | Notes |
|---|---|
| `aws_acm_certificate.edge` (+ validation records + validation) | `*.staging.packiot.app`, SAN `staging.packiot.app`. us-east-1. DNS-validated into the staging child zone. |
| `aws_wafv2_web_acl.edge` | scope CLOUDFRONT, us-east-1. Rate-limit (block) + 4 AWS managed groups (in **count** mode by default). |
| `aws_cloudfront_distribution.edge` | alias `*.staging.packiot.app`; origin `origin.staging.packiot.app`; CachingDisabled + AllViewer; WAF attached. |
| `aws_route53_record.origin` | `origin.staging.packiot.app` A → App EIP. The non-fronted hostname CloudFront dials. |
| `random_password.origin_verify` | Shared secret CloudFront stamps as `X-Origin-Verify`. |

None of these change where `<svc>.staging.packiot.app` resolves and none change the
...
|---|---|---|
| `edge_cutover` | `false` | `true` → `<svc>.staging.packiot.app` become ALIAS → CloudFront (the A→EIP copies go dormant). **DNS cutover.** |
...
### (a) Apply with everything off — creates the edge, zero staging impact

```bash
cd terraform/staging
make tf-init   # (or terraform init with the staging backend)
terraform apply
...
> ⚠️ The ACM validation needs `staging.packiot.app` NS delegation live in the parent
...
Add to **each** `server { … }` block that serves a `*.staging.packiot.app` vhost
...
> the check is live, **direct** browser hits to `<svc>.staging.packiot.app` (still
...
> `auth.staging.packiot.app`, `mq.staging.packiot.app`, `amqp.staging.packiot.app`
> and `cpack-ingest.staging.packiot.app` vhosts are NOT fronted by CloudFront (see
> "Out of scope" below) — do **not** add the check to those server blocks or you'll
> 403 legitimate SSO / RabbitMQ / AMQPS / CPACK-ingest traffic.
>
...
through `auth.staging.packiot.app` (still direct — see scope note).

**Rollback:** `terraform apply -var 'edge_cutover=false'` → back to A→EIP.

### (e) Enforce the WAF managed rule groups

After watching CloudWatch (`packiot-staging-edge-*` metrics) + sampled requests for
...
`https://<eip>` is refused at the network layer. Ports 22 / 80 / 5671 / 8447 are
untouched (80 stays open for Let's Encrypt http-01 renewals; 5671 AMQPS + 8447
CPACK-ingest are separate non-443 listeners).

> ⚠️ **Before flipping this**, resolve `auth.staging.packiot.app` (and any other
> direct-to-origin 443 vhost, e.g. `mq.staging.packiot.app`). `auth` is **not** in
> the cutover set, so it still resolves A→EIP. Once 443 is CloudFront-only, direct
> traffic to `auth.staging` is dropped and SSO breaks. Either (1) add `auth` to the
> CloudFront path first — its hostname is already covered by the
> `*.staging.packiot.app` alias + cert, so you only need to point its Route53 record
> at CloudFront — or (2) leave 443 open and rely on the `X-Origin-Verify` check for
> origin protection instead of the SG lock. Pick one deliberately; do not flip the
> lock with `auth` (or `mq`) still direct-to-EIP.
>
> Note `mq.staging.packiot.app` is a 443 HTTP vhost (RabbitMQ management proxy) that
> is a SEPARATE Route53 record, NOT in `var.services` — so `edge_cutover` never
> fronts it. If you want it behind the edge you must add it deliberately (it's
> already covered by the wildcard cert + alias); otherwise it too must stay reachable
> when you flip the lock.
...
- **`auth.staging.packiot.app`** — NOT in the cutover set (`dns.tf`
  ...
  before `edge_origin_lock=true`** (see step (f) warning). Same caveat as production's
  `auth.prod`.
- **`mq.staging.packiot.app`** — RabbitMQ management HTTP proxy on 443, a separate
  Route53 record NOT in `var.services`, so `edge_cutover` never fronts it. Stays
  A→EIP unless you add it deliberately.
- **`amqp.staging.packiot.app:5671`** — AMQPS, TCP not HTTP. Must **never** go
  through CloudFront. Stays A→EIP; the 5671 SG rule is untouched. Do not add the
  nginx `X-Origin-Verify` check to it. (Production has no AMQPS equivalent — its
  factory uplink is the mTLS `ingest.prod` :8883 record.)
- **`cpack-ingest.staging.packiot.app:8447`** — ADR-0042 P1 TLS reverse-proxy for the
  sparkplug-agent `/v1/tags` endpoint, admitted from CPACK's egress /32 only. Not a
  standard 443 vhost; stays A→EIP, SG rule untouched. Do not front it and do not add
  the `X-Origin-Verify` check. (Analogous to production's `ingest.prod` :8883
  exclusion.)
