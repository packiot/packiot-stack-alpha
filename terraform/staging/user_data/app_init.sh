...
HASURA_JWT=$(echo "$HASURA_SECRET"   | jq -r '.jwt_secret')
...
HASURA_GRAPHQL_JWT_SECRET=$HASURA_JWT
HASURA_GRAPHQL_ENABLE_CONSOLE=true
HASURA_GRAPHQL_DEV_MODE=false

EDGE_API_KEY=$API_KEY
EDGE_API_URL=http://api.$STAGING_DOMAIN

RABBITMQ_USER=$MQ_USER
RABBITMQ_PASSWORD=$MQ_PASS
RABBITMQ_URL=amqp://$MQ_USER:$MQ_PASS@localhost:5672
...
# This is a stub — replace with the actual compose.staging.yml from the repo.
# Clone the repo here and use the staging compose file.
cd /opt/packiot
git clone https://github.com/$GITHUB_REPO.git stack 2>/dev/null || {
  cd stack && git pull
}
