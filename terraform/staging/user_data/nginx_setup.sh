...
server {
    ...
    # /session/* — edge-api does its OWN Cognito-Bearer auth (login.service authenticate),
    # so bypass the SSO gate exactly like /api/. Without this the front4 super-admin
    # enterprise switcher's GET /session/enterprises hits the shell gate (302 to oauth2/start)
    # and never reaches edge-api, so the picker is empty and the switcher stays hidden.
    location ~ ^/session(/|\$) {
        proxy_pass         http://127.0.0.1:8080;
        proxy_set_header   Host              \$host;
        proxy_set_header   X-Real-IP         \$remote_addr;
...
# ── operator-bispharma vhost (Bispharma operator — enterprise 5) ──────────────────
# Twin of operator-sbx: proxies to the operator-bispharma container on :8087, which
# injects the BISPHARMA enterprise api-key, so writes land on ent 5 (never CPACK).
cat > /etc/nginx/conf.d/operator-bispharma.conf <<NGINX
server {
    listen 80;
    server_name operator-bispharma.$STAGING_DOMAIN;
    return 301 https://\$host\$request_uri;
}
server {
    listen 443 ssl;
...
server {
    ...
    location / {
        ...
        if (\$request_method = OPTIONS) {
            ...
            add_header Access-Control-Allow-Headers "Authorization, X-Api-Key, Content-Type, x-packiot-agent, x-user, x-operator-superadmin-token" always;
...
map \$http_origin \$scan_cors_allow_origin {
    ...
    "https://operator-bispharma.$STAGING_DOMAIN" \$http_origin;
}
