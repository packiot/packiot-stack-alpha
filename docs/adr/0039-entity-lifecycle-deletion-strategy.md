...
**Built on staging (foundation + history rows) — the two-migration split:**
- **R6 — `reference/migrations/0039-dimension-scd2-temporal.sql`** (the temporal *columns*): added `valid_from`/`valid_to`/`created_at`/`updated_at` + a `set_updated_at()` trigger to `equipments`/`sites`/`areas`/`enterprises`. This is the SCD-1→SCD-2 column foundation; it records *when the current version took effect* but still overwrites the prior version.
- **R6 follow-up — `reference/migrations/0039-dimension-scd2-history.sql`** (the history *rows*): the missing half. A `<dim>_history` table (`LIKE` the dimension + a `history_id` surrogate PK + `changed_at`) per dimension, written by a shared BEFORE-UPDATE trigger (`trg_scd2_history` → `log_dimension_history()`) that snapshots the superseded row via `to_jsonb(OLD)`/`jsonb_populate_record` (column-count-agnostic — `equipments` has 51 columns), closes its interval (`valid_to := now()`), and rolls the live row's `valid_from` forward so history + live **tile** the timeline. A change-guard (compare `to_jsonb(OLD)` vs `to_jsonb(NEW)` minus the temporal columns) suppresses spurious versions from no-op/pure-touch updates. Applied + proven on `packiot_shadow` (a self-rolled-back test UPDATE mints exactly one history row holding the OLD values with `valid_to = changed_at`; the live `valid_from` advances; a no-op UPDATE mints none). Additive + reversible; **prod GATED.**
- **Still open for Tier 2:** `packml_register` / `production_orders` / `shifts` — they have no temporal columns yet (R6 covered only the four dimensions above); they join once R6 is extended to them (§6 phase T2), reusing the same generic `log_dimension_history()` trigger.
...
| **T2** | SCD-2 `*_history` (trigger-written or system-versioning) for `equipments`, `packml_register`, `production_orders`, `shifts`. **Lands with 0038 Phase-B3 Traceability.** *Built on staging:* temporal columns (`0039-dimension-scd2-temporal.sql`, R6) + `*_history` tables & capture trigger (`0039-dimension-scd2-history.sql`) for the four dimensions that have temporal columns; `packml_register`/`production_orders`/`shifts` pending R6 extension. | Med–High | Yes (history is additive; base tables unchanged) |
