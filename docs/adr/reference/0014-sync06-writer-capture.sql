-- update_piot_table_production_data_sync_enterprise_06 — captured
-- 2026-07-03 (READ ONLY, chunked substr fetch, 20,609 bytes).
--
-- STRUCTURE (for the port session): a transaction-status STATE
-- MACHINE over production_data_sync_enterprise_06 — rows carry
-- trans_status (N=new / D=delete / U=update?) + to_delete + logics
-- (numbered condition that last touched the row) + real_update.
-- Compute source: get_data_sync_enterprsie_06b(21) (21-day window;
-- the b-generation — call-time-verify before porting, dead-generation
-- rule!). Conditions observed: 0 = resurrect (existed→deleted→back:
-- reset to N, to_delete=false), 1 = insert new packiotid (with
-- jobstatus 'in_progress'→'in progress' normalization), … (read the
-- body). Companion writer: *_70days variant (20.9KB) — likely the
-- deep-window rebuild pass (fetch at port time).
--
-- PORT NOTES: consumer = the enterprise-6 sync export (analogous to
-- sap_13 but WITHOUT the external back4-api coordination — this one
-- is fully PL/pgSQL-driven, so it is unblocked). Pool target per
-- ADR-0012 Wave 1 pattern; natural key = packiotid.
