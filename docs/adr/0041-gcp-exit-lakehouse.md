...
**Status:** Proposed · **Date:** 2026-07-22 (rev. 2026-07-23 — added §10 concrete P0→P3 build plan + reference build-plan doc + unrooted Terraform scaffold; rev. 2026-07-27 — added §11 QuickSight serve tier = AWS-native PowerBI replacement + companion `reference/designs/0041-quicksight-bi.md`) · **Scope:** DESIGN / SCOPING ONLY (read-only inventory + target architecture + phased cutover + P0 scaffold; **no infra is provisioned, no prod is touched, no GCP resource is mutated by this ADR — the Terraform module is unrooted and applies nothing**). STAGING-first; every GCP-teardown / prod-touching step is explicitly **USER-gated**. · **Decision owner:** data/platform architect (pending USER review).
...
## 11. QuickSight serve tier — the AWS-native PowerBI replacement (completes the analytics stack)

§3–§10 replace the **BigQuery *backend*** (the query engine) with S3 + Glue + Athena. But BigQuery was never the whole GCP-vs-Microsoft analytics story: the stack's **visualization/BI tier is Microsoft PowerBI**, running `.pbix` files on customer machines that **ODBC-DirectQuery straight into prod `tsp12`** through a dedicated `powerbi` role. Retiring BigQuery without retiring PowerBI would leave the analytics stack *half* AWS-native and still hammering the production OLTP DB. **Amazon QuickSight is the serve/render tier that closes the loop** — QuickSight datasets on the lakehouse's Athena, replacing PowerBI's charts and the ODBC-to-prod load, keeping the same visuals.

**Full design:** [`reference/designs/0041-quicksight-bi.md`](reference/designs/0041-quicksight-bi.md) — the migration surface (the 37 PowerBI-facing objects + consumers), the QuickSight architecture (SPICE vs Direct-Query, tag-based RLS on `id_enterprise`, front4 embedding via the Cognito path), the visual-parity mapping, the phased cutover, the cost model, and the **P4** build plan + Terraform surface. ADR-level headline below.

**Where it sits:** this is the medallion's **serve** (of *store → transform → serve*, ADR-0036 §2.4) — one tier *above* Athena. It reads the **Gold** datasets (build-plan §2.3), not Timescale. The full AWS-native analytics stack becomes **Bronze → S3 → Glue → Athena → QuickSight → (embedded in) front4** — every tier inside the one-cloud/one-IAM boundary of [ADR-0038 §5.9](0038-north-star-factory-platform.md). This extends the single-cloud principle from *infra* to *analytics-and-BI*: after this, **no analytics tier depends on GCP (BigQuery) or Microsoft (PowerBI licensing + client-side `.pbix`)**.

**The isolation invariant holds by construction, at five layers.** `id_enterprise` is the same key as: the Cognito token claim (ADR-0034) → the QuickSight embed **session tag** → the dataset **tag-based RLS** rule → the **first S3 partition key** (build-plan §1) → the Athena `WHERE` predicate. Tenant isolation (ADR-0038 §5.5) and cost-control (partition pruning) become *the same mechanism* — a correctly-isolated query is automatically the cheapest scan, and a reader can never even scan another tenant's partitions.

**Visual-parity verdict:** *yes — the standard OEE visuals port cleanly* (KPI, gauge, bar, line, table, pivot, filter controls, drill-down, drill-through-as-actions all have direct QuickSight equivalents). **One real gap:** the 24 h **state-timeline / Gantt** (`c35_dashboard_timeline_24h`) has **no native QuickSight visual** — use a stacked-bar pseudo-Gantt or leave it in Grafana. Plus two rebuild-effort items (DAX measures → QuickSight calculated fields; audit for any marketplace custom visuals) and one out-of-scope item (the `sap_report_data_sync_*` **SAP feeds are ETL, not visuals** — they stay a data pipeline, repointed to Athena separately).

**Migration = rebuild, not import** (no PowerBI→QuickSight converter exists): build in parallel → validate side-by-side vs live PowerBI (reuse the `powerbi-compatibility-test-plan.md` parity discipline, retargeted to QuickSight-on-Athena) → cut customers over one-by-one with per-customer sign-off → retire the prod `powerbi` role + ODBC (which also removes the DirectQuery load from prod Timescale — the audited 1.1 GB-temp / 88-min / 5-connection footprint). **Complementary to Grafana** (Grafana = operational/real-time ops; QuickSight = business/analytical BI — non-overlapping) and **beside front4's own composition engine** (P10), not replacing it.

| Phase | Name | What lands | Prod/external touch | Exit criteria |
|---|---|---|---|---|
| **P4a** | **Subscribe + wire** | QuickSight **Enterprise** (RLS + embedding both require it); Athena data source; datasets on Gold (+`cq_logs`); **tag-based RLS** on `id_enterprise`; **SPICE** import + daily refresh tied to the export cadence. Scaffold `terraform/modules/quicksight/` (unrooted). | **No** (new AWS only) | Datasets refresh green; RLS returns only the tagged tenant; SPICE refresh fires post-export. |
| **P4b** | **Rebuild dashboards** | Rebuild the 37-object set as QuickSight analyses→dashboards (per customer); DAX→calculated fields; capture as template JSON. **Parity-validate** vs PowerBI. | **No** (build); parity read SELECT-only | Every object matches PowerBI (row zero-drift + sum in-band); timeline gap resolved. |
| **P4c** | **Embed in front4** | Embed endpoint (refdata-api/Lambda) minting `GenerateEmbedUrlForAnonymousUser` with the `id_enterprise` session tag from the verified Cognito claim; front4 iframe via the embedding SDK; origin allow-list. | **No** (staging) | A staging Cognito user sees only their tenant's dashboard, RLS-enforced. |
| **P4d** | **Cut over + retire PowerBI** | Cut customers one-by-one (per-customer sign-off); drop the prod `powerbi` role + ODBC DirectQuery path. | **Yes — USER-gated** (external + prod role) | All customers signed off; PowerBI ODBC decommissioned; prod DirectQuery load gone. |

**Cost:** Enterprise edition (mandatory for RLS + embedding) — a few author seats (~$18/seat/mo) + embedded reader sessions (metered $0.30/session or a session-capacity bucket) + small SPICE storage; realistically **~$40–120/mo** at pilot scale, folding into the lakehouse footprint (SPICE caps Athena at one scan/dataset/day). Offset by **retiring PowerBI licensing** *and* **removing the prod-DB DirectQuery load**. (Aside: **Amazon Managed Grafana** ~$9/editor + $5/viewer per mo is on record if the user later wants hosted Grafana too — orthogonal, out of scope.)

**Gating:** P4 builds **after Athena exists (P2) and the F2 refactor flip settles** (so Gold is the canonical post-refactor shape). P4a–P4c are **staging + new-AWS-only, autonomous-eligible**; only **P4d** (external customer cutover + prod `powerbi`-role drop) is **USER-gated**.
