...
dir, **not** a new container / front-door / DNS record / SG rule (ADR-0047). The box side
is just a reader POSTing the rawtag envelope to the one shared front-door
...
`equipments` — those columns are required or the stream-engine reads the topic as "not
