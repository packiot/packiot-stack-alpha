...
## QUALITY FIX — L6 register+TRIG map RECOVERED from legacy :1881 (2026-08-18)
Node-RED topology on the CPACK box: :1880 = new-stack ADR-0045/csadmin-generated reader
(feeds our sparkplug-agent; writable, no auth); :1881 = LEGACY reader (oracle; admin-auth,
creds admin/****); :1882 = another new-stack instance (localhost-bound, not reachable).
Read :1881 via `POST /admin/auth/token` (client_id=node-red-admin) → `GET /admin/flows`.

Root cause of the Quality inflation: **cpack.plc.yaml's `PLC_L6` modbus block had ZERO
tags** → the generated reader defaulted L6 counters to register 0, single 16-bit, no TRIG
→ garbage + net=gross. (Confirmed: F3 L6-PTH 10-day sum gross 811M/net 206M = wrapped
16-bit reads.) The legacy :1881 "Prepare L6" + "PackML device config" give the authoritative
map (32-bit pairs, low-word-first; ***TRIG_CS on outfeed processed = scrap):
| tag | HR | topic | TRIG |
|-----|----|-------|------|
| BREYER  | 60  | ProdConsumedCount/91  | C=O |
| POLYTYPE| 10  | ProdConsumedCount/93  | C=O |
| PTH     | 100 | ProdConsumedCount/95  | — |
| PTH     | 50  | ProdProcessedCount/95 | CS |
| RMH     | 50  | ProdConsumedCount/94  | — |
| RMH     | 0   | ProdProcessedCount/94 | CS |
| TEXA    | 0   | ProdConsumedCount/92  | — |
| TEXA    | 0   | ProdProcessedCount/92 | CS |
Serial flow: PTH-out(reg50)=RMH-in; RMH-out(reg0)=TEXA-in/out. Line net=reg0 (TEXA),
line gross=reg50 (RMH) → Q≈116504/130447≈0.89.

**DONE:** populated `cpack.plc.yaml` PLC_L6 with the 8 tags above (address=HR int, kind=holding,
type=int32, word_swap=true). Decode already ports TRIG faithfully, so once the reader emits
these tags WITH ***TRIG_CS from the right registers, Quality matches the oracle.
**REMAINING to go live:** regenerate the :1880 reader from the updated SSoT via the csadmin
generate step (validate the generate step's modbus address encoding vs this HR-int form),
deploy to :1880, verify against oracle (F3 L6-TEXA per-row gross≠net; line Q≈0.89). L5 needs
the same treatment (its S7 block still has raw numeric tag names 17/18/… instead of topics+TRIG).

## DEPLOYED LIVE + VALIDATED (2026-08-18 21:49 UTC)
Root cause was the Node-RED reader-flow GENERATOR (PR #810): qty defaulted to 1 (never
derived 2 for uint32) + normalize took p[0] only → 32-bit totalizers read as single 16-bit
→ garbage (F3 L6-PTH 10-day sum was 811M) → net=gross → Q pinned at 1.0.
Live fix: surgically patched the running :1880 reader (new-stack, csadmin/ADR-0045-generated,
no auth) via POST /admin/flows — set qty=2 on the 8 L6 count reads + added the low-word-first
combine (p[1]*65536+p[0]) to the normalize fn, KEEPING the live env vars (CPACK_AGENT_URL/
CPACK_INGEST_KEY — the generated reader uses AGENT_INGEST_API_KEY, so a wholesale replace
would have broken the POST). Backup saved before deploy. HTTP 204.
VALIDATED: (1) L6-RMH per-row gross−net now NON-ZERO (was exactly 0 every row — the smoking
gun); (2) values sane 32-bit (no 811M garbage); (3) reader healthy, data flowing continuously;
(4) our L6 line TRACKS the oracle (gross 378≈oracle 377; Q moves with real production, both
~1.0 in low-scrap windows vs the old permanent 1.0). The qty-change totalizer jump was
absorbed by the spike guard (one settling row). Minor residual: our line-net = TEXA reg0 is a
slight simplification of the legacy's full TRIG_CS defective-accumulation; empirically they
track within counter-timing noise. Durable path = merge #810, regenerate, redeploy (the live
patch matches what #810 now generates).

## L8/L10 OUTFEED METER CORRECTION — TEXA@DINT20 is STALE, net meter = TCX (task #59, 2026-08-26)
The earlier backfill set L8/L10 outfeed = **TEXA (222 / 567)** per "net meter = TEXA". That is
oracle-correct *semantically* but the **reader register for TEXA is stale on the current PLC**,
so on OUR new-stack twin the TEXA count-index carries no usable value. Pinned end-to-end on the
live new-stack path (reader → cpack-ingest/v2/tags → sparkplug-agent-cpack → decoder → F3):

| stage | L8/TEXA | L10/TEXA | verdict |
|-------|---------|----------|---------|
| reader emits (tcpdump @agent :9104) | yes, `DB1,DINT20` | yes, `DB1,DINT20` | reader OK |
| **value read** | **626329** (unrelated ctr) | **0** (empty reg) | **register stale** |
| agent births + publishes | yes (in NBIRTH) | yes | agent OK (birth-fix live) |
| decoder decodes | yes | yes | decode OK |
| oracle TEXA totalizer | 217796 | 254927 | ≠ our DINT20 |

**Key proof:** oracle `TEXA(222)≡TCX(221)` and `TEXA(567)≡TCX(566)` byte-for-byte over 24h
(max val diff 0; identical incr sums 19384 / 146732) → TEXA and TCX are the SAME outfeed count.
Our reader's **TCX @ `DB1,DINT16`** matches the oracle (217502≈217796, 254543≈254927) and TCX
(id 75/79) already flows to F3 matching the oracle line net. So the true outfeed value lives at
`DB1,DINT16` (TCX); `DB1,DINT20` (TEXA) is a stale mapping (PLC-program drift since the Jul-23
vartable) — it reads junk on L8, empty on L10. This is NOT an alias/birth drop (the whole
new-stack pipeline carries TEXA fine — the value is simply wrong).

**APPLIED (live, verified):** repointed the L8/L10 LINE outfeed counter `222→221`, `567→566`
(`docs/clients/cpack-l8l10-outfeed-counter-fix.sql`). Corrected meters:
- **L8 = 219 / 221** (DXL infeed / **TCX** outfeed) · **L10 = 564 / 566** (DXL / **TCX**).
- Line net populated within one Phase-9 reseed cycle: **L8 0→917 vs oracle 910 (~1%)**,
  **L10 0→1098 vs oracle 1191** (uniform ~8% on gross too → poll-cadence skew, not metering).
- L4/L6 not regressed. The TEXA machine (76/80) stays dark (cosmetic). Reversible.
