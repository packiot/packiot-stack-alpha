...
- **R8 — `proportional_target` proration asymmetry (MEDIUM, data-correctness).** Two sibling procs deliberately differ, and it is easy to "reconcile" them into a bug. Verified live 2026-07-18 (#83, SELECT-only `pg_proc`): base `piot_get_equipment_runtime_shift_production` (tp=1 machines / tp=2 sectors) keeps the elapsed-proration write #2 **COMMENTED** → full-shift `proportional_target` (write #1 `vl_target` is the sole writer); `_tp_eq3` (tp=3 lines) has write #2 **ACTIVE** → elapsed-prorated. The committed parity SQL (`edge-node-red/db/20-oee-engine-parity.sql`) already matches prod exactly. → The Calc/Go cutover **must preserve this asymmetry**; do **NOT** uncomment base write #2 to "match" the line behavior — that double-writes `proportional_target` (single-writer-per-column violation) and **resurrects #80**. Related: front4's client override is redundant for tp=3 lines and being removed (#80); whether machines *should* also be elapsed-prorated is a separate backend product question, not a cutover reconciliation. Secondary: base fn reads cagg `ca_agg_equipment_values_1hour_fast` in prod vs `ca_agg_equipment_values_1hour` in the file — verify vs the shadow schema before any change. **Owner: dba + backend.**
