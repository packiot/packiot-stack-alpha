...
>   (b) Postgres **RLS** is a DB-level fail-closed backstop keyed on
>   `SET app.tenant_id`. Superset RLS is the PRIMARY enforcer (§3).
...
- `db/superset/01-superset-ro-role.sql` — read-only role + curated `bi.*` views
  (ported 1:1 from the Metabase design; only the role name changes).
- `db/superset/02-tenant-rls.sql` — Postgres RLS backstop keyed on `app.tenant_id`.
- `db/superset/03-authoring-rls-mapping.optional.sql` — optional `bi.user_tenant`
  mapping table for authoring-RLS **Strategy B** (§2.2).
- `db/superset/README.md` — the two-isolation-layers explainer.
- `configs/superset/superset_config.py` — the Superset config sketch (§1.5).
...
`db/superset/02-tenant-rls.sql` — RLS on the analytics tables, policy
`USING (id_enterprise = current_tenant())` where `current_tenant()` reads the
`app.tenant_id` session GUC. **The same honesty as the Metabase spec:** Superset
uses **one pooled SQLAlchemy connection per registered database**, shared across
all users and guest tokens. It does **not** run `SET app.tenant_id` per query. So
under the default single-connection topology `current_tenant()` is NULL and the
policy **denies all rows (fail-closed)** — Postgres RLS is a **backstop, not a
co-enforcer**; **Superset RLS is doing the real per-tenant work.**

| Topology | How GUC is set | Postgres RLS role | Ops cost |
|---|---|---|---|
| **(c) single pooled `superset_ro`** (default) | never set → `current_tenant()` NULL → deny-all | **fail-closed backstop** (a sandbox-less/mis-scoped query returns zero rows, not cross-tenant data) | zero |
| **(a) per-tenant Superset DB entry** | connection string `?options=-c app.tenant_id=<id>` | **co-enforcer** — Postgres RLS + Superset RLS both bite | one Superset "database" + connection per tenant |
| **(b) pooled proxy stamps it** | a thin proxy runs `SET app.tenant_id` per session | co-enforcer | build/run a proxy |

Default **(c)**: Superset RLS primary, Postgres RLS fail-closed net. Moving to
**(a)** upgrades the backstop to a co-enforcer at the cost of a connection per
tenant — the biggest multi-tenancy decision (§6).
