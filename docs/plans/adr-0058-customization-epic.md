...
- **P1.4b — reader-publishes-sensors (DEFERRED, needs a design pass).** A derive that references a sensor with an offset but NO member (the example's original `S1 - S2`) currently errors loudly. Making it work means the generator synthesizes a raw reader tag for that sensor — but the §C `checkClientAgentConsistency` invariant then forces that raw tag into the agent allowlist (and thus onto the cloud uplink), OR needs a new "consume this derive-input, don't republish" path in the deriver. That's the real remaining decision. Deferred until the reader-publishes semantics + a real client that needs a non-member operand are confirmed. Original recon (kept for reference):
