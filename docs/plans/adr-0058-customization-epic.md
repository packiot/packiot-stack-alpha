...
- **P1.4 — wire ADR-0050 `plc.types[].derive`** (currently parsed-but-unexecuted) to P1.2 — its first real customer. **⚠ Not a drop-in — needs one design decision first (ADR-0050 §4).** Recon findings (grounded in `clientdescriptor.go` / `generate.go`):
