...
Status: **PLAN / design-only (2026-09-08).** Nothing in prod auth was changed producing this
doc — it is READ-ONLY hardproof + a sequenced migration design. Owner: platform.
...
## 10. AUTHORING PHASE — prod artifacts written (2026-09-08). **AWAITING USER GO TO APPLY.**

User decision this phase: *"Author it all as reviewable PRs, STOP before apply."* Everything
below is committed code / SQL files / a patch. **Nothing was applied. No `terraform apply`, no
DDL/DML on `packiot40`, no deploy, no front4 flip.** All hardproof is read-only (validate/build/test).

### 10.1 What was authored (all Path B, one dedicated prod pool `packiot-prod`)

| # | Deliverable | Location | Proof |
|---|---|---|---|
| 1 | Prod Cognito pool + front4 client + hosted-UI domain | `terraform/production/cognito.tf` | `terraform validate` = **Success** (module-wide, with the unrelated pre-existing `ci_runner.tf` em-dash issue set aside) |
| 2 | Prod migrate-on-login Lambda + `packiot/prod/firebase-web-api-key` secret placeholder, `MIGRATION_ENABLED=false` | `terraform/production/cognito_migration_lambda.tf` | archive provider added to `main.tf`; validate clean |
|   | Cognito outputs (pool id / client id / issuer / jwks) | `terraform/production/outputs.tf` | " |
| 3 | `packiot40` `ADD COLUMN id_user_cognito` + partial-unique index CONCURRENTLY (+ down) | `db/migrations/packiot40-cognito-user-id/01_*.up.sql` / `.down.sql` | authored, not run |
| 4 | 7-unlinkable-user remediation (2 dup pairs + 5 no-email), guarded tx + row-count asserts | `db/migrations/packiot40-cognito-user-id/02_remediate_7_unlinkable_users.sql` | authored, not run |
| 5 | primary-api dual-accept Cognito strategy (mirror of back4-api) | `docs/plans/patches/159-primary-api-cognito-dual-accept.patch` | `nest build` clean, **24 auth tests green** on `packiot/api@main` |

Companion (pre-existing, from §8.1): **back4-api** dual-accept is on branch
`feat/cognito-dual-accept` @ `e7bbaa9` (dark by default, 11 tests). Together back4-api + the
primary-api patch cover both legacy Node relying parties.

### 10.2 Key decisions taken (confirm or override)

- **Dedicated prod pool `packiot-prod`** (NOT the staging pool `packiot-staging`). `deletion_protection = ACTIVE` (staging is INACTIVE) — a customer pool must not be `terraform destroy`-able by accident.
- **oauth2-proxy stays OUT of this pool.** The prod internal-admin SSO already authenticates against the *staging* pool (`user_data/app_init.sh: COGNITO_USER_POOL_ID=us-east-1_0T9t1sTwt`). `packiot-prod` is exclusively the **front4 customer** plane. Hosted-UI domain `packiot-prod-auth` is provisioned anyway for future hosted forgot-password + a possible later oauth2-proxy migration.
- **Tenant is resolved from the DB, never a token claim.** Hardproof: the migrate-on-login Lambda's `buildUserAttributes` sets only `email/email_verified/name/firebase_uid` — **no `custom:id_enterprise`**. So both Node relying parties resolve by `users.id_user_cognito` + single-row link-on-login, exactly like read-api. (primary-api's Firebase leg reads `id_enterprise` from the token today; its Cognito leg cannot and must not.)
- **7-user remediation defaults (need your confirm):** deactivate the *later* id of each dup pair — **641** (keep 609) and **612** (keep 507) — to match the link guard's `ORDER BY id_user ASC`. The 5 ent-99 no-email actives (**964–968**): **recommend deactivate** (Option A) unless STEP 0's audit shows a real active identity, in which case pre-provision by `sub` (Option B). *Both options are in the SQL; STEP 0 is a read-only audit to run first.*
- **`packiot/api` is archived (read-only)** → primary-api is a **patch**, not a PR. Confirm whether primary-api is still deployed / whether it should instead be retired (Path A) rather than taught Cognito.

### 10.3 Finalized apply sequence (each step + its rollback) — DO NOT run without go

**Everything before step 8 is a config/flag revert. Step 8 (front4-prod flip) is the one
human-green-light, customer-visible action. Step 11 (DROP COLUMN) is the only irreversible one.**

| # | Action | Rollback |
|---|---|---|
| 0 | Architect confirms: pool `packiot-prod` name, dual-accept (Path B), 7-user dispositions above. | — |
| 1 | `terraform apply` `cognito.tf` + `cognito_migration_lambda.tf` (prod workspace). Pool + client + inert Lambda (`MIGRATION_ENABLED=false`) created. **No user impact.** | `terraform destroy -target` the new resources (flip `deletion_protection=INACTIVE` first). |
| 2 | Run `db/migrations/packiot40-cognito-user-id/01_*.up.sql` on `packiot40` (NO `--single-transaction`). Additive column + index. | `01_*.down.sql`. |
| 3 | Run `02_*` **STEP 0 audit**, confirm, then STEP 1 (+ STEP 2 A or B). | `active=true` on the deactivated ids. |
| 4 | Deploy back4-api (`feat/cognito-dual-accept`) + primary-api (apply the patch) with `COGNITO_*` set but `COGNITO_AUTH_ENABLED` **OFF**. Deploy is behavior-neutral (dark). | Redeploy prior image / unset env. |
| 5 | Set `COGNITO_ISSUER` + `COGNITO_CLIENT_ID` on `edge-api-prod-docker-env` (code already dual-accept). Leave `FIREBASE_ISSUER` unset. | Unset the two env vars. |
| 6 | Flip `COGNITO_AUTH_ENABLED=true` on back4-api + primary-api. Still no Cognito tokens arriving yet → inert. | Set flag off. |
| 7 | Populate `packiot/prod/firebase-web-api-key` secret, then set the prod Lambda `MIGRATION_ENABLED=true`. Still inert until a Cognito login is attempted. | `MIGRATION_ENABLED=false`. |
| **8** | **★ HUMAN GREEN-LIGHT ★ front4 `.env.production`: set `VITE_AUTH_COGNITO_ENABLED=true` + pool id + client id (`VITE_COGNITO_LINK_ENABLED=false`); build + deploy prod.** First login per user → JIT-migrate + server link. | **Revert the 3 vars + rebuild → instant Firebase restore.** Migrated users keep working (backend still dual-accept). |
| 9 | Soak: dashboard `SELECT count(*) FROM users WHERE active AND id_user_firebase IS NOT NULL AND id_user_cognito IS NULL` on `packiot40` → drive to **0**. Pre-provision the tail (AdminCreateUser SUPPRESS + set `id_user_cognito`). | (still fully reversible) |
| 10 | Once 0 for a clean window: `FIREBASE_ISSUER` off (already unset on edge-api-prod), then the §5 Firebase-removal PR set (staging-proven first). | Re-enable Firebase leg. |
| 11 | **Irreversible:** `ALTER TABLE users DROP COLUMN id_user_firebase` on `packiot40` — separate deploy, gated on a zero-Firebase-reader log-watch. **Rotate + scrub the leaked back4-api `private-key.json` (task #222).** | none — do only after the soak. |

### 10.4 The one gate to remember

> **AUTHORED — AWAITING USER GO TO APPLY.** No prod resource, DB row, or deploy has changed.
> The customer base is 100% Firebase and stays that way until **step 8 (front4-prod flip)**,
> which is reserved for an explicit human green-light because it is the customer-visible flip
> and slow to reverse. Never flip Firebase off (step 10/11) before the step-9 count reaches 0.
</content>
</invoke>
