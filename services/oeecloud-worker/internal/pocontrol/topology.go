...
const topoPackmlSeq = `UPDATE %s.packml_register SET line_unit_seq = $1 WHERE packml_topic = $2`
...
const topoLineRow = `
	...
	VALUES ($1, $2, $3, $4, $5,
	        (SELECT id_equipment FROM %[1]s.packml_register WHERE id_site = $3 AND id_unit = $6 GROUP BY id_equipment),
	        (SELECT id_equipment FROM %[1]s.packml_register WHERE id_site = $3 AND id_unit = $7 GROUP BY id_equipment))
...
const topoMemberRow = `
	...
	VALUES ($1, $2, $3, $4,
	        (SELECT id_equipment FROM %[1]s.packml_register WHERE id_site = $3 AND id_unit = $5 GROUP BY id_equipment),
	        $6, $7, $8, $9)
	ON CONFLICT (ts_value, id_equipment) DO UPDATE SET
...
// executeTopology runs the 30700 unit in one tx.
func (h *Handler) executeTopology(ctx context.Context, pool *pgxpool.Pool, m *sparkplug.Metric, schema string) error {
	...
	if _, err := tx.Exec(ctx, fmt.Sprintf(topoPackmlSeq, schema),
		strings.Join(seq, ","), m.TopicForRegister()); err != nil {
		return fmt.Errorf("packml seq: %w", err)
	}
	if _, err := tx.Exec(ctx, fmt.Sprintf(topoLineRow, schema),
	...
	if _, err := tx.Exec(ctx, fmt.Sprintf(topoZeroLine, schema), ts, info.IDEquipment); err != nil {
	...
	for i, unit := range unitOrder {
		...
		if _, err := tx.Exec(ctx, fmt.Sprintf(topoMemberRow, schema),
	...
	if _, err := tx.Exec(ctx, fmt.Sprintf(topoZeroMembers, schema), ts, info.IDEquipment); err != nil {
