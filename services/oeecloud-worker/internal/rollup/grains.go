...
func LoopGrains(ctx context.Context, dests []flows.Dest, exclAreas, exclEnterprises, machineLevelEnterprises []int, shiftLimit int, dqEnabled, clampEnabled, changeoverAvailability bool, every time.Duration, logger *slog.Logger, obs jobs.Observer) {
	...
	jobs.Loop(ctx, jobs.Job{Name: "runtime-rollup", Every: every, Run: func(ctx context.Context) error {
		...
		for _, d := range dests {
			...
			if err := RunHour(ctx, d, exclAreas, exclEnterprises, changeoverAvailability); err != nil {
			...
			if n, err := RunShift(ctx, d, exclAreas, exclEnterprises, machineLevelEnterprises, shiftLimit, changeoverAvailability); err != nil {
