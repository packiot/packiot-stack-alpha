...
--Isso criava problemas, pois acabava criando inumeros inputs da logica 4
	    update customer_reports.production_data_sync
	    set to_delete = false,
	    	trans_status = 'N',
	    	logics = 0,
			real_update = now()
...
-- Condition 1: Insert records where packiotid does not exist in the table
    ...
    SELECT
		...
		1 as logics,
		now() as real_update
    FROM serving.data_sync(__ENTERPRISE__, 21)
    WHERE packiotid NOT IN (
        SELECT packiotid
        FROM customer_reports.production_data_sync
        WHERE shiftstartdate >= NOW() - INTERVAL '23 day'
        AND customer_id = __ENTERPRISE__
    );
    ...
    SELECT
		...
		5 as logics,
		now() as real_update
    FROM serving.data_sync(__ENTERPRISE__, 21) gds
    LEFT JOIN customer_reports.production_data_sync pds
    ...
    AND pds.customer_id = __ENTERPRISE__
...
WHERE customer_reports.production_data_sync.packiotid = subquery.packiotid
and customer_reports.production_data_sync.trans_status = 'N'
and customer_reports.production_data_sync.shiftstartdate >= now() - interval '23 day'
and customer_reports.production_data_sync.customer_id = __ENTERPRISE__;
...
/*UPDATE customer_reports.production_data_sync psde
SET
	jobstatus = CASE
...
WHERE
    ...
    AND po.status > 2
    AND po.id_enterprise = __ENTERPRISE__
...
with po_runtimes as (
   ...
   where por.id_equipment in (select id_equipment from equipments where id_enterprise = __ENTERPRISE__ and tp_equipment = 3)
   ...
   select
   	...
   	por.ts_end
   from customer_reports.production_data_sync pds
...
UPDATE customer_reports.production_data_sync pds
SET
...
where
    ...
    and customer_reports.production_data_sync.shiftstartdate >= now() - interval '21 day'
    and (customer_reports.production_data_sync.supervisorapproveddate is null or customer_reports.production_data_sync.supervisorapproveddate != evs.ts_user_validation)
    and customer_reports.production_data_sync.packiotid = evs.index1
    and customer_reports.production_data_sync.customer_id = __ENTERPRISE__;
...
WHERE customer_reports.production_data_sync.indice_geral = calculated_data.indice_geral
AND customer_reports.production_data_sync.customer_id = __ENTERPRISE__
AND (customer_reports.production_data_sync.final_trans_status is null or customer_reports.production_data_sync.final_trans_status != calculated_data.f_trans_status);
...
WITH results AS (
    ...
    WHERE id_enterprise = __ENTERPRISE__
    AND id_order IN (
        SELECT DISTINCT job
...
UPDATE customer_reports.production_data_sync pdse
...
AND pdse.customer_id = __ENTERPRISE__
