...
//
// AMQP user `stream-engine` is least-privilege on staging RabbitMQ:
// it can declare its own topology, consume from stream-engine-q, and
...
type Config struct {
	...
	RabbitMQSecretID string // packiot/staging/rabbitmq-stream-engine-creds
	...
	WorkerQueue    string // 'stream-engine-q' (new, this worker only)
	RetryExchange  string // 'oee-retry' (NEW — DLX target for nacks)
	RetryQueue     string // 'stream-engine-q-retry-30s' (NEW — TTL 30s, DLX back to oee)
	FailedExchange string // 'oee-failed' (NEW — terminal, human inspection)
	FailedQueue    string // 'stream-engine-q-failed' (NEW — no TTL, no DLX)
...
func Load() (*Config, error) {
	return &Config{
		...
		RabbitMQSecretID:                 getenv("RABBITMQ_SECRET_ID", "packiot/staging/rabbitmq-stream-engine-creds"),
		AMQPHost:                         getenv("RABBITMQ_HOST", "rabbitmq"),
		AMQPPort:                         getenvInt("RABBITMQ_PORT", 5672),
		SourceExchange:                   getenv("SOURCE_EXCHANGE", "oee"),
		WorkerQueue:                      getenv("WORKER_QUEUE", "stream-engine-q"),
		RetryExchange:                    getenv("RETRY_EXCHANGE", "oee-retry"),
		RetryQueue:                       getenv("RETRY_QUEUE", "stream-engine-q-retry-30s"),
		FailedExchange:                   getenv("FAILED_EXCHANGE", "oee-failed"),
		FailedQueue:                      getenv("FAILED_QUEUE", "stream-engine-q-failed"),
