...
func TestSentinelReportFailed(t *testing.T) {
	cases := []struct {
		...
		{"clean surface passes",
			SentinelReport{Overflow: []OverflowOutcome{{Surface: "equipment_oee_shift", Enterprise: 3, Violations: nil}}}, false},
		{"multiple clean surfaces pass",
			SentinelReport{Overflow: []OverflowOutcome{
				{Surface: "equipment_oee_shift", Enterprise: 3},
				{Surface: "production_orders_runtime", Enterprise: 3},
			}}, false},
		{"a violation fails the gate (the L8 112M overflow class)",
			SentinelReport{Overflow: []OverflowOutcome{
				{Surface: "equipment_oee_shift", Enterprise: 3, Violations: []string{
...
func TestSentinelReportString(t *testing.T) {
	clean := (&SentinelReport{Overflow: []OverflowOutcome{{Surface: "equipment_oee_shift", Enterprise: 3}}}).String()
	...
	dirty := (&SentinelReport{Overflow: []OverflowOutcome{
		{Surface: "equipment_oee_shift", Enterprise: 3, Violations: []string{"a", "b"}},
	}}).String()
