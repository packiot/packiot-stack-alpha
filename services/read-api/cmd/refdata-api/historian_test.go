...
func TestHistorianSQLShape(t *testing.T) {
	for _, m := range []string{
		"FROM cold.equipment_values_all",                 // the VIEW (mixed hot+cold), t287 `cold` schema; NOT the ev_between function
...
func TestHistorianDowntimeSQLShape(t *testing.T) {
	for _, m := range []string{
		"FROM cold.equipment_events_all",          // the EE hot+cold union VIEW, t287 `cold` schema
		"id_enterprise = $1",               // tenant fence on the SERVER-resolved cid
