...
//
// SCOPE OF THE PIN (§3c shadow-diff now CLOSED — see external.go):
// the ENVELOPE and the column-ordered row shape are pinned exactly from source.
// The fixture rows now ALSO include the numeric/decimal + bigint columns that
// were previously excluded: node-postgres returns those as JSON STRINGS (scale
// preserved, e.g. "0.00"/"12.34"/"9007199254740993"), and pgxExternalReader forces
// their OIDs onto the text wire and passes the raw Postgres text through as a Go
// string, so the fixture value for such a column is the string node-pg delivers.
// The live column typing was read SELECT-only from prod (information_schema /
// \gdesc); the deterministic serialization divergence and the exact node-pg
// strings were proven against prod (BEGIN READ ONLY). float8/int4 stay JSON
// numbers on both sides (no fix needed) and are pinned as such where present.
...
// compareGolden asserts body is byte-identical to the frozen golden for
// (consumer,name). With UPDATE_GOLDEN=1 it (re)writes the golden instead — the
// same regeneration workflow as TestContractGolden. This lets the numeric
// fixtures be regenerated deterministically from the shim's own output; the
// regenerated bytes are then reviewed to confirm they match node-pg (numeric/
// bigint quoted with the exact Postgres scale, float8/int4 unquoted).
func compareGolden(t *testing.T, consumer, name, body string) {
	t.Helper()
	...
	b, err := os.ReadFile(p)
	if err != nil {
		t.Fatalf("read golden %s (run UPDATE_GOLDEN=1 to create): %v", p, err)
	}
	if body != string(b) {
		t.Errorf("%s/%s body not byte-identical to frozen golden.\n got:  %s\n want: %s",
			consumer, name, body, string(b))
	}
}
...
func TestNeopacSapReportGoldenShape(t *testing.T) {
	...
	reader := &scriptedReader{fn: func(sql string, args []any) (externalRows, error) {
		...
		// >2^53 to prove precision-preserving passthrough; gyartasi_ido is
		// numeric(10,2) → "12.34"; total_dt is a ZERO numeric(10,2) → "0.00" (NOT
		// "0" — the scale-drop pgtype.Numeric would otherwise cause). The reader
		// delivers these as Go strings, exactly as node-pg does.
		return externalRows{
			cols: []string{"id_equipment", "werk", "material", "menge_soll", "bezeichnung", "erstellt", "job", "gyartasi_ido", "total_dt"},
			rows: [][]any{{42, "1300", "MAT-1", 1000, "Kübel groß <A&B>", time.Date(2026, 7, 18, 6, 0, 0, 0, time.UTC), "9007199254740993", "12.34", "0.00"}},
		}, nil
	}}
	...
	compareGolden(t, "neopac", "sap-report", body)
}
...
func TestNeopacSapReportSyncGoldenShape(t *testing.T) {
	...
	reader := &scriptedReader{fn: func(sql string, args []any) (externalRows, error) {
		...
		return externalRows{
			...
			rows: [][]any{
				{"4711", "128000", "L01", "2026-07-18", "F", 1, "480.00", "0.00", time.Date(2026, 7, 18, 6, 0, 0, 0, time.UTC)},
				{"4712", "96500", "L02", "2026-07-18", "S", 2, "233.40", "12.00", time.Date(2026, 7, 18, 14, 0, 0, 0, time.UTC)},
			},
		}, nil
	}}
	...
	compareGolden(t, "neopac", "sap-report-sync", body)
}
