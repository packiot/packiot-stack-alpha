...
// A 16-bit totalizer near its ceiling (65000/65535) wraps to a small value
// (100). ON: increment is the rollover delta (65535-65000)+100 = 635, the
// counter persists at the new post-wrap reading, and it is NOT a reset. OFF:
// same input rebaselines to 0 → increment 100 (the undercount the ADR names).
func TestCounterRollover_WrapVsReset(t *testing.T) {
	const cmax, prev, cur = 65535, 65000, 100
	const wantRollover = (cmax - prev) + cur // 635
...
// path (byte-identical to off). Prevents guessing a wrap width.
func TestCounterRollover_NoCounterMaxFallsBackToReset(t *testing.T) {
	const prev, cur = 65000, 100
