...
//
// SCOPE (honest boundary, same as NEOPAC): fixtures use only string/int/bool/
// time.Time values whose Go↔node-pg JSON serialization is pinnable. Numeric/
// decimal columns (duration, net/gross_production, presscount) are DELIBERATELY
// absent — node-pg stringifies numeric and the column typing is a live-DDL
// property, so that parity is the §3c step-3 SHADOW-DIFF, not asserted here.
...
func TestMontebelloDataSyncGoldenShape(t *testing.T) {
	...
	reader := &scriptedReader{fn: func(sql string, args []any) (externalRows, error) {
		return externalRows{
			...
			rows: [][]any{{"MTB-SITE", "Linha 3 <A&B>", 4711, tsUTC(6, 0, 0), "spBv1.0/mtb/DDATA/L01"}},
		}, nil
	}}
	...
	if want := readGolden(t, "montebello", "data-sync"); body != want {
		t.Errorf("data-sync body not byte-identical to frozen golden.\n got:  %s\n want: %s", body, want)
	}
}
...
func TestMontebelloEventsGoldenShape(t *testing.T) {
	...
	if want := readGolden(t, "montebello", "events"); body != want {
		t.Errorf("events_montebello body not byte-identical.\n got:  %s\n want: %s", body, want)
	}
}
...
func TestIncoplastEventsGoldenShape(t *testing.T) {
	...
	if want := readGolden(t, "incoplast", "events"); body != want {
		t.Errorf("events_incoplast body not byte-identical.\n got:  %s\n want: %s", body, want)
	}
}
...
func TestIncoplastJobsGoldenShape(t *testing.T) {
	...
	reader := &scriptedReader{fn: func(sql string, args []any) (externalRows, error) {
		...
		return externalRows{
			cols: []string{"id_production_order", "id_order", "ts_start", "ts_end", "id_equipment", "status", "topic", "last_update"},
			rows: [][]any{{9001, "OP-778", tsUTC(6, 0, 0), tsUTC(14, 0, 0), 42, 3, "spBv1.0/inco/DDATA/EX2", tsUTC(14, 0, 5)}},
		}, nil
	}}
	...
	if want := readGolden(t, "incoplast", "jobs"); body != want {
		t.Errorf("jobs_incoplast body not byte-identical.\n got:  %s\n want: %s", body, want)
	}
}
