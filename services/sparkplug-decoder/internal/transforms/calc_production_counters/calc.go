...
func CalcWithConfig(msg Message, state State, cfg Config) (Decision, error) {
	...
	case CounterKindProcessed:
		...
		if prevProcessed > curProcessed {
			// ADR-0037 (f) flag-gated per-equipment totalizer wrap takes
			// precedence; on no-match fall back to the staging 16-bit-wrap /
			// genuine-reset classifier (ADR-0048). Flag OFF ⇒ rolloverAdjustedPrev
			// returns (0,false) ⇒ the fallback path runs byte-identically.
			if newPrev, isRollover := rolloverAdjustedPrev(prevProcessed, curProcessed, counterMax, cfg); isRollover {
				// A rollover is NOT a reset (production continues), so it must
				// NOT rebaseline line scrap nor trip the reset-heal.
	...
	case CounterKindConsumed:
		...
		if prevConsumed > curConsumed {
			if newPrev, isRollover := rolloverAdjustedPrev(prevConsumed, curConsumed, counterMax, cfg); isRollover {
	...
	case CounterKindDefective:
		...
		if prevDefective > curDefective {
			...
			if newPrev, isRollover := rolloverAdjustedPrev(prevDefective, curDefective, counterMax, cfg); isRollover {
