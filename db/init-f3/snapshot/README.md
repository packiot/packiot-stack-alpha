...
against the live-`packiot_shadow` target manifest (see ../README.md §5/§6).

| File | Applied | What |
|---|---|---|
| `00-packiot_shadow-schema.sql` | best-effort | curated schema-only `pg_dump` of staging `packiot_shadow` — all 152 tables, 129 user functions, 10 views (byte-parity). Cagg-view/`_timescaledb_internal`/cross-schema/debris blocks stripped (recreated strict below or intentionally dropped). |
...
| `10-f3-timescale-supplement.sql` | strict | the 3 remaining raw hypertables (`equipment_events`, `equipment_values_raw`, `equipment_events_raw`) + the 5 `ca_*` continuous aggregates. Definitions introspected SELECT-only from `packiot_shadow`. |
| `15-f3-read-api-composite-type-fixes.sql` | strict | fixes 2 porting bugs `00` inherited from live `packiot_shadow` that 500'd 9 read-api `/v1/query` datasets (docs/audits/front4-operator-endpoint-health.md): 8 `h_piot_*`/`h_*` row-shape tables had a jsonb-aggregate column typo'd `text[]` instead of `jsonb[]` (F1 parity confirmed via live `pg_attribute` diff), and `h_piot_get_mission_control_timeline` read `agg_equipment_values_1min_t` — a name already owned by an unrelated, differently-shaped F3 view — instead of the correctly-shaped `agg_equipment_values_1min`. Applied live to staging `packiot_shadow` 2026-08-20 (idempotent: safe to re-run, safe to drop once a `00` regeneration captures the fix at the source). |
