...
--
-- (Ported verbatim-in-intent from db/metabase/02-tenant-rls.sql — the DB-level
--  backstop is BI-tool-agnostic; role name superset_ro replaces metabase_ro.)
--
-- The DB-LEVEL BACKSTOP beneath Superset's own Row-Level-Security. Postgres RLS
-- keyed on a per-connection session GUC `app.tenant_id`. Any connection reading
-- these tables sees ONLY rows whose id_enterprise matches the GUC — regardless of
-- what the querying client (Superset, psql, a mis-scoped BI tool) intended.
--
...
--       (Superset: one "database" entry per tenant — clean RLS, N connections).
--   (b) per-session SET:  SET app.tenant_id = '<id>';  before each query
--       (works for a bespoke pooled proxy that stamps it from the token).
--   (c) role default:  ALTER ROLE superset_ro_t<id> SET app.tenant_id = '<id>';
--       (one login role per tenant; the GUC rides the role).
-- Under a SINGLE pooled superset_ro connection with NO GUC (Superset's default —
-- ONE SQLAlchemy connection per registered database, shared across all users and
-- guest tokens), current_setting() below returns '' → the policy denies ALL rows
-- (fail-closed). That is why, in the default single-connection topology, the
-- Superset RLS (guest-token clause for viewers, role-bound filter for authors) is
-- the PRIMARY enforcement and this Postgres RLS is a genuine backstop only under
-- (a)/(b)/(c). See the spec §3 + §6 — this is the biggest multi-tenancy decision.
...
-- superset_ro is a normal (non-BYPASSRLS) role — confirm it never bypasses.
