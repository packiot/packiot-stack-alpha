-- t284 — eliminate the 3 LIVE public.h_* SRF carriers by making their producing
-- functions self-describing (RETURNS SETOF <table>  ->  RETURNS TABLE(<cols>)).
-- Target DB: packiot_analytics (10.10.10.89)
--
-- Consumer-transparent: the CALL signature and result columns are unchanged.
-- read-api wraps each as `SELECT * FROM fn($1..$n)`; the DB output is what matters
-- and it is byte-identical (proven — see README.md HARDPROOF).
--
-- Each body is the VERBATIM original with ONE addition: `#variable_conflict
-- use_column` as the first line. RETURNS TABLE introduces the result columns as
-- OUT-parameter *variables*; without the directive they shadow the same-named
-- table columns (id_equipment, id_enterprise, ...) and plpgsql's default
-- variable_conflict=error aborts the call. `use_column` forces column precedence
-- — restoring EXACTLY the resolution the original SETOF-of-composite function had
-- (it declared no OUT variables, so columns always won). Hence identical output.
--
-- Return-type change requires DROP before CREATE (CREATE OR REPLACE cannot alter
-- a function's return type). No object hard-depends on these 3 functions
-- (pg_depend: 0 non-return-type dependents), and serving.downtime_by_category
-- resolves its helper by name at call time, so the drop/recreate is safe.
...
CREATE OR REPLACE FUNCTION public.h_piot_get_downtimes_per_category_equipment_level_new_4(in_id_enterprise integer, in_ids_sites text, in_ids_areas text, in_ids_equipments text, in_ids_shifts text, _tsstart timestamp without time zone DEFAULT date_trunc('month'::text, now()), _tsend timestamp without time zone DEFAULT now(), in_ids_teams text DEFAULT '{}'::text)
 RETURNS TABLE(id_enterprise integer, duration_microstops bigint, duration_total bigint, duration_justified bigint, duration_planned bigint, duration_unplanned bigint, available_time bigint, downtimes_per_category text[])
 LANGUAGE plpgsql
 STABLE
AS $function$
#variable_conflict use_column
