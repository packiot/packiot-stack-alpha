-- t-events-view-line-attribution-dedupe — front4 Events tab listed each attributed stop TWICE.
--
-- Lead-machine attribution writes each stop on the lead machine (tp=1) AND its line (tp=3).
-- serving.v_events_2 (only consumer: serving.events_timeline_full → read-api events-timeline-full →
-- front4 EventsTab) maps every event to its top parent, so both copies rendered as the line: one
-- justified (the LINE copy — tp=3 carries every justification: 732/1,339 this week) and one
-- "pending" (the machine copy — tp=1 justified 0/2,990).
-- FIX: the type-1 branch drops a MACHINE event only when its top-parent line has an EXACT mirror
-- (same ts_event AND status; PK lookup). Machine stops without a line mirror stay visible.
-- PROOF (staging, CPACK, front4 window 09-17..09-23, rolled-back tx): rows 2,499 → 1,886;
-- justified rows 617 → 617 (none lost); new ⊆ old; view dependency set identical; owner postgres +
-- security_invoker=on preserved; same count as readapi_ro. Remaining same-key rows (27) are two
-- DIFFERENT member machines stopping at the same instant — both roll up to the line label (view
-- design, unchanged).
BEGIN;
SET LOCAL search_path TO "$user", gold, silver, bronze, identity, config, ops, serving, customer_reports, core, public;
CREATE OR REPLACE VIEW serving.v_events_2 WITH (security_invoker = on) AS
 SELECT 1 AS event_type,
  ...
  WHERE (ee.duration >= COALESCE(e.stop_threshold_time, 0) OR ee.ts_end IS NULL) AND ee.status <> 6 AND e.event_should_be_displayed = true
    AND NOT (e.tp_equipment = 1 AND COALESCE(ppe.id_equipment, pe.id_equipment) IS NOT NULL AND EXISTS (
          SELECT 1 FROM equipment_events le
           WHERE le.id_equipment = COALESCE(ppe.id_equipment, pe.id_equipment)
             AND le.ts_event = ee.ts_event
             AND le.status = ee.status))
UNION
