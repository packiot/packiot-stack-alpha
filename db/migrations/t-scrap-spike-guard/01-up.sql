-- t-scrap-spike-guard
--
-- front4 "Scrap by Period" showed ~90% scrap for a few weeks (Aug 2026). Root cause
-- was a faulty PLC on line L5 (and a few others) recording physically-impossible
-- scrap — 1–4 MILLION scrap per shift vs ~20–80K good units (the same L5 whose speed
-- also spiked to 2714). The scrap %% formula is correct; it faithfully showed garbage.
--
-- This adds a physical-plausibility guard to the HISTORICAL branch of the scrap/
-- single-period RPC: a shift is dropped when its scrap exceeds ~150%% of the line's
-- maximum possible output (production_speed units/min × available_time min). No line
-- can scrap more than it could produce, so that value can only be a counter artifact.
-- Only touches impossible rows (cpack: 31 shifts); plausible scrap is unchanged
