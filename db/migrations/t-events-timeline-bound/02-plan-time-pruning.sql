-- t-events-timeline-bound / 02 — PLAN-TIME chunk pruning for the operator event feeds.
-- Root cause (2026-09-24): silver.equipment_events has 1-DAY chunks; the T1 history
-- backfill took it from ~120 to 1,599 chunks. These functions were inlined SQL with
-- `now() - interval …` bounds → TimescaleDB could only exclude chunks at EXECUTION, the
-- planner still built paths for all 1,599: events_timeline planned 22.8 s / ran 96 ms.
-- RETURN QUERY EXECUTE + literal bounds (format %L) → excluded at PLAN time.
-- Same row types, same predicates, same semantics. (Chunk consolidation of the
-- history = separate maintenance: merge_chunks, pinned.)
