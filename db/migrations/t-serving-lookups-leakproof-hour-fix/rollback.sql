-- rollback: the six functions before t-serving-lookups-leakproof-hour-fix (verbatim), and drop the
-- ported shift-hour function (it did not exist in packiot_analytics before → HOUR grain 500s again).
-- Indexes are additive and harmless; drop only if you really want the old state:
--   DROP INDEX CONCURRENTLY IF EXISTS gold.idx_er1h_equipment_prod_day;
BEGIN;
...
DROP FUNCTION IF EXISTS public.piot_get_shift_hour_by_equipment_fixed(integer, integer, timestamp with time zone);
COMMIT;
