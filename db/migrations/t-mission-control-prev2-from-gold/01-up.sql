-- t-mission-control-prev2-from-gold — Mission Control 'null' previous-previous shift.
--
-- APPLY AFTER t-mission-control-lead-timeline (#1449); the body below includes it.
--
-- serving.mission_control read prev2_shift_name / prev2shift_netprod / prev2_target from
-- silver.equipment_live_shift, whose writer never populates prev2_* (NULL for every line, both
-- tenants) → front4's card showed 'null' for the shift before last. Take prev2 from gold with the
-- same semantics the live prev1_* follow (verified 43/43: prev1 = 2nd-latest gold shift row →
-- net, target, shifts.cd_shift), i.e. prev2 = 3rd-latest; the live value still wins if filled.
-- Staging: prev2 filled 20/20 CPACK, 23/23 Bispharma (e.g. L01 T1 / T3 / T2 24,007); every
-- other column identical (drift-checked both call orders).
BEGIN;
