-- t-serving-group-by-case-insensitive — shift/team breakdowns silently collapsed via read-api.
--
-- APPLY AFTER t-serving-lookups-leakproof-hour-fix (#1436): the bodies below include its changes.
--
-- SYMPTOM: front4 Total Production / Single Period / Targets / Machine Speed "by shift" / "by team"
-- views showed ONE enterprise-wide series (or null cd_shift/id_shift) instead of per-shift rows.
-- ROOT CAUSE: read-api validates enum filters case-insensitively and passes them LOWERCASED
-- (datasets.go `s = strings.ToLower(s)` → 'shifts'), but these functions — written for the legacy
-- Hasura path, which sent 'SHIFTS' — compare with `case group_by_element when 'SHIFTS' ...`
-- (total_production_by_team: UPPER() in one spot, bare `partitionBy` in the jsonb key + window
-- partition). 'shifts' matched nothing → fell to the enterprise/general branch.
-- FIX: normalize once at the top of each body: `group_by_element := upper(group_by_element)`
-- (`partitionBy` for total_production_by_team). Case-insensitive for every caller; upper-case
-- callers unchanged.
-- PROOF (staging, 2026-09-24): for 138 cases (5 fns × day/week/month/hour × shifts/teams/none ×
-- CPACK+Bispharma × windows) md5(new(lowercase)) == md5(old(UPPERCASE)) and
-- md5(new(UPPERCASE)) == md5(old(UPPERCASE)).
BEGIN;
