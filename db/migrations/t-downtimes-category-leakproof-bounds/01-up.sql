-- t-downtimes-category-leakproof-bounds — the last ~1 s of front4 Downtimes "Downtime Reasons".
--
-- After t-rls-initplan-policies / -ers-window / -microstops-po-join, CPACK "This Month" as
-- readapi_ro still spent ~2–2.7 s in serving.downtime_by_category. Profile (auto_explain,
-- nested statements) showed three separate costs, none of them the aggregation itself (~50 ms):
--
-- 1. min_ts_prod `select min(ts_value) ... where ts_value_production between ..` (~300 ms):
--    the planner rewrites min() as "first row of equipment_oee_shift_ts_value_idx" and walked
--    ~670k shift rows backwards filtering on tenant + production day. max(ts_end) (no index
--    on ts_end) fell back to a similar scan. FIX: index (id_equipment, ts_value_production)
--    — max drops to ~10 ms — and wrap the min in a MATERIALIZED CTE so it uses it too.
-- 2. ers_w CTEs (~224–285 ms each, computed twice): `ts_value > (timestamp expr)` — ts_value is
--    timestamptz, so that's the cross-type operator timestamptz_gt_timestamp, which is NOT
--    LEAKPROOF → under RLS it can only be a Filter, never an Index Cond (26k heap fetches).
--    FIX: cast the bound ::timestamptz (same conversion the cross-type operator does, session
--    TimeZone) → timestamptz_gt (leakproof) → index range scan: 224 → 48 ms.
-- 3. downtime_by_category's `ers.id_shift = any(ids_shifts)` and `ers.ts_range && window`
--    were applied to the CTE SCAN, where there are no statistics → default selectivities →
--    estimate 1 row (actual 5,642). With the faster CTE the planner flipped to a nested loop
--    rescanning the stops function 5,642× (24 M join rows; O(N×M)). FIX: move both filters
--    INTO ers_w, onto the real table (column stats). Inner join → identical result.
--
-- PROOF (staging, 2026-09-24): md5 of full outputs orig vs new identical for CPACK month /
-- front4-month / single-shift / day / week / 2023-03 / 2025-12, sector_view, Bispharma (ent 5),
-- and as readapi_ro under RLS. Interleaved timing readapi_ro, CPACK Sept (front4 args):
-- orig 1.97–2.76 s → new 0.93–1.31 s. Plan: Hash Join restored.
--
-- Index: applied live with CREATE INDEX CONCURRENTLY (276 MB table); plain IF NOT EXISTS here
...
declare
	...
	-- MATERIALIZED: stops the planner's min()→"first row of the ts_value index" rewrite, which
	-- walked ~670k shift rows backwards under RLS; fetch by idx_ers_equipment_prod_day, then min.
	min_ts_prod timestamp := (with w as materialized (select ev.ts_value from equipment_oee_shift ev
...
WITH ers_w AS MATERIALIZED (
  SELECT * FROM equipment_oee_shift
   WHERE id_equipment IN (SELECT id_equipment FROM equipments WHERE id_enterprise = in_id_enterprise)
     AND ts_value > (_tsstart::timestamp - interval '1 months' - interval '1 day')::timestamptz  -- tz-typed: timestamptz>timestamp is not LEAKPROOF → no index cond under RLS
)
...
-- (_tsstart - 1 day, _tsend)). Bounds are tz-typed: timestamptz > timestamp is not LEAKPROOF,
-- so under RLS it can't be an index condition. The shift/overlap filters live HERE, on the real
-- table (column stats → sane row estimate); applied to the CTE scan they got default
-- selectivities (estimate 1 row vs ~5.6k) and the planner nested-looped the stops × shifts.
WITH ers_w AS MATERIALIZED (
  SELECT * FROM equipment_oee_shift
   WHERE id_equipment IN (SELECT id_equipment FROM equipments WHERE id_enterprise = in_id_enterprise)
     AND ts_value > (_tsstart - interval '1 day')::timestamptz AND ts_value < _tsend::timestamptz
     AND ts_range && tstzrange(_tsstart ,_tsend) --eduardo 2024-07-13 (nao estava funcionando para paradas longas)
     AND id_shift = any( ids_shifts )
) 	
select distinct 
	ee.id_enterprise,
	array_agg(
	...
	join ers_w ers 
		on (
			...
			and (
				...
				-- (window overlap + shift filter: applied inside ers_w)
						)
