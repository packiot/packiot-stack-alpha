-- rollback: definitions before t-downtimes-category-leakproof-bounds (verbatim). The index is
-- additive and harmless; drop it only if you really want the old state:
--   DROP INDEX CONCURRENTLY IF EXISTS gold.idx_ers_equipment_prod_day;
BEGIN;
