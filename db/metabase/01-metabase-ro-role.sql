-- db/metabase/01-metabase-ro-role.sql
-- W2 embedded-Metabase scaffolding — INERT until the W2 go decision.
-- Apply by hand, staging-first. See db/metabase/README.md + docs/plans/w2-embedded-metabase.md.
--
-- Creates:
--   * a least-privilege login role `metabase_ro` (SELECT-only, no DDL, no writes),
--   * a `bi` schema of CURATED, tenant-safe VIEWS that Metabase connects to.
--
...
-- churn, and (c) guarantee every exposed row carries `id_enterprise` so the
-- Metabase sandbox + the RLS backstop both have a column to key on.
--
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'metabase_ro') THEN
    ...
    -- (packiot/<env>/app key metabase_db_ro_password) at apply time, e.g.:
    --   ALTER ROLE metabase_ro WITH PASSWORD :'pw';
    CREATE ROLE metabase_ro WITH LOGIN PASSWORD 'CHANGE_ME_AT_APPLY';
  END IF;
END$$;

-- Never let this role escalate. Explicitly no CREATEDB/CREATEROLE/SUPERUSER.
ALTER ROLE metabase_ro NOSUPERUSER NOCREATEDB NOCREATEROLE NOINHERIT;

-- ── 2. Curated BI schema ──────────────────────────────────────────────────────
CREATE SCHEMA IF NOT EXISTS bi AUTHORIZATION CURRENT_USER;

-- metabase_ro may look inside `bi` and read its views, and read the base tables
-- ONLY through the views (the views run as their owner = the applying superuser,
-- so metabase_ro needs no direct base-table grant — least privilege).
GRANT USAGE ON SCHEMA bi TO metabase_ro;

-- Curated OEE aggregate — shift grain. Every row carries id_enterprise (resolved
-- via the equipments dimension) so both the Metabase sandbox and the RLS backstop
-- have a tenant key. Trim to the columns a factory analyst actually builds on.
...
JOIN equipments eq ON eq.id_equipment = rs.id_equipment
JOIN equipments e  ON e.id_equipment  = rs.id_equipment;  -- id_enterprise source
...
GRANT SELECT ON ALL TABLES IN SCHEMA bi TO metabase_ro;
