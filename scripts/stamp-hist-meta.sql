-- stamp-hist-meta.sql — R5. Stamp hist_meta.last_append_at for every ev_promoted
-- enterprise. Run by the append job's post-run hook (historian-staging-run-append.sh)
-- IMMEDIATELY AFTER a successful cold-store append and BEFORE refresh-hist-cutover.sql,
...
-- after the last boundary refresh ⇒ ev_all is double-counting the newly-archived window.
--
-- Metadata-only (reads hist_promoted_enterprise); no parquet scan. Idempotent.
INSERT INTO hist_meta (id_enterprise, last_append_at, source, updated_at)
SELECT id_enterprise, now(), 'historian-append-hook', now()
  FROM hist_promoted_enterprise
 WHERE ev_promoted
