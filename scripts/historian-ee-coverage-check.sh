#!/usr/bin/env bash
# historian-ee-coverage-check.sh — R7. Guard the equipment_events_all HOT-ANCHORED
# completeness caveat.
#
# WHY (10-historian-gateway.sh EE header): equipment_events_all hands the whole overlap
...
# (equipment_events, bounded to recent months for a cheap prune) MUST be a subset of the set
...
EE_ENTS="$(psql -tAc "SELECT id_enterprise FROM promoted_enterprise WHERE ee_promoted ORDER BY 1" | sed '/^$/d')"
if [ -z "$EE_ENTS" ]; then
