-- stamp-equipment_values-meta.sql — R5. Stamp cold_append_watermark.last_append_at for every ev_promoted
...
-- so a present refresh always leaves ev_union_boundary.refreshed_at >= cold_append_watermark.last_append_at.
--
-- The staleness monitor (scripts/historian-staleness-monitor.sh, R4) then flags a MISSED
-- refresh hook cheaply: last_append_at > ev_union_boundary.refreshed_at ⇒ the cold store grew
...
INSERT INTO cold_append_watermark (id_enterprise, last_append_at, source, updated_at)
...
ON CONFLICT (id_enterprise)
  DO UPDATE SET last_append_at = EXCLUDED.last_append_at,
                ...
                updated_at     = now();
SELECT id_enterprise, last_append_at FROM cold_append_watermark ORDER BY id_enterprise;
