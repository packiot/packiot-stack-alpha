...
# prod by dumping the SCHEMA (data-free) of staging's live packiot_analytics and
...
# packiot_analytics (measured F3_MISSING=344 + EXTRA=377 — see db/init-f3/README.md
# §3). A schema-only dump FROM packiot_analytics matches it by construction.
#
...
if [ "${CONFIRM:-}" != "yes" ]; then
  cat >&2 <<EOF
GATED. This reads staging packiot_analytics's SCHEMA (data-free, read-only-in-effect)
...
script -qec "aws ssm start-session --target $INSTANCE \
  ...
  | tr -d '\n' | base64 -d | gunzip > "$OUT/00-packiot_analytics-schema.sql"

echo "wrote $OUT/00-packiot_analytics-schema.sql ($(wc -l < "$OUT/00-packiot_analytics-schema.sql") lines)" >&2
...
python3 - "$OUT/00-packiot_analytics-schema.sql" <<'PY'
import re, sys
