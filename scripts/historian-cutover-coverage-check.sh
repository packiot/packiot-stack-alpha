#!/usr/bin/env bash
# historian-cutover-coverage-check.sh — CI / boot assertion for the ev_union_boundary invariant.
#
# INVARIANT (t271, load-bearing — see services/historian-gateway/docker-entrypoint-initdb.d/
# 10-historian-gateway.sh): the ev_union_boundary boundary set MUST equal the EV-promoted
...
# enterprise that has a ev_union_boundary row, so:
...
#   * a ev_union_boundary row for a NON-promoted enterprise => that tenant's hot history is
...
# archive to ev_union_boundary; that is obsolete — the cold archive is no longer served
...
# Current ev_union_boundary boundary set.
cut_ents="$(docker exec -i "$CONTAINER" psql -U postgres -d packiot_historian -tAc \
  'SELECT id_enterprise FROM ev_union_boundary ORDER BY 1' | sed '/^$/d' | sort -un)"
...
rc=0
if [ -n "$missing" ]; then
  echo "COVERAGE VIOLATION: ev_promoted enterprise(s) with NO ev_union_boundary row (DOUBLE-COUNT risk):" >&2
  printf '  enterprise=%s\n' $missing >&2
  echo "Fix: run services/historian-gateway/refresh-equipment_values-cutover.sql on the gateway." >&2
  rc=1
fi
if [ -n "$extra" ]; then
  echo "COVERAGE VIOLATION: ev_union_boundary row(s) for NON-ev_promoted enterprise(s) (hot-CLIP risk):" >&2
  printf '  enterprise=%s\n' $extra >&2
  echo "Fix: DELETE FROM ev_union_boundary WHERE id_enterprise NOT IN (SELECT id_enterprise FROM promoted_enterprise WHERE ev_promoted);" >&2
  rc=1
fi
[ "$rc" -eq 0 ] && echo "ev_union_boundary coverage OK: boundary set == ev_promoted allow-list ($(printf '%s\n' "$promoted" | sed '/^$/d' | wc -l | tr -d ' ') enterprise(s))."
exit "$rc"
